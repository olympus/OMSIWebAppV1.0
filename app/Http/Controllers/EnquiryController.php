...
use App\DataTables\EnquiryRequestsDataTable;
...
{
    public function indexOld(EnquiryRequestsDataTable $dataTable)
    {
        return $dataTable->render('enquiry.index',['export_path'=>'all', 'page_name'=>'All']);
    }

    public function index(Request $request, $type = 'ALL')
    {
        $data['page_name'] = $type;
        return view('enquiry.index',['export_path'=> $type, 'page_name'=> $type]);
    }

    public function enquiryRequestList(Request $request)
    {
        ...
        if($request->from_date && $request->to_date && $request->data_type == 'archive_data'){
            ...
            try {
                ...
                $totalArchiveData = ArchiveServiceRequests::where('request_type','enquiry')->whereIn('status', $page_name)->
                    ...
                    ->count();
                $totalCurrentData = ServiceRequests::where('request_type','enquiry')->whereIn('status', $page_name)->
                    with('hospital','departmentData','employeeData','customer')
                ...
                if(empty($request->input('search.value'))){  //filter for archive or current financial year. data without search
                    $archive_data = ArchiveServiceRequests::where('request_type','enquiry')->whereIn('status', $page_name)->
                        with('hospital','departmentData','employeeData','customer')
                        ->where('archive_service_requests.is_practice', false)
                        ->whereBetween('archive_service_requests.created_at', [$from_date, $to_date])
                        ->orderBy('archive_service_requests.id',$dir)
                        ->get();
                    $current_data = ServiceRequests::where('request_type','enquiry')->whereIn('status', $page_name)->
                        with('hospital','departmentData','employeeData','customer')
                        ->where('service_requests.is_practice', false)
                        ->whereBetween('service_requests.created_at', [$from_date, $to_date])
            ...
            try {
                ...
                $totalData = ServiceRequests::where('request_type','enquiry')->whereIn('status', $page_name)->
                    with('hospital','departmentData','employeeData','customer')
                    ->where('service_requests.is_practice', false)
                    ->whereBetween('service_requests.created_at', [$from_date, $to_date])
                    ->count();
                ...
                if(empty($request->input('search.value'))){
                    ...
                    $posts = ServiceRequests::where('request_type','enquiry')->whereIn('status', $page_name)->offset($start)
                        ->limit($limit)
                        ->with('hospital','departmentData','employeeData','customer')
                        ->where('service_requests.is_practice', false)
                        ->whereBetween('service_requests.created_at', [$from_date, $to_date])
        ...
        try{ //in this step data is pass for a datatable
            ...
            if(!empty($posts))
            {
                foreach ($posts as $current_user) {
                    ...
                    if(Auth::user()->isA('superadministrator|administrator|administratorenquiry')){
                        $nestedData['options'] = "
    ...
    public function indexByType(EnquiryRequestsDataTable $dataTable, $type)
    {
        ...
        switch ($type){
            case 'received' :
                return $dataTable->render('enquiry.index',['export_path'=>'received', 'page_name'=>'Received']);
                break;
            case 'assigned' :
                return $dataTable->render('enquiry.index',['export_path'=>'assigned', 'page_name'=>'Assigned']);
                break;
            case 'attended' :
                return $dataTable->render('enquiry.index',['export_path'=>'attended', 'page_name'=>'Attended']);
                break;
            case 'closed' :
                return $dataTable->render('enquiry.index',['export_path'=>'closed', 'page_name'=>'Closed']);
                break;
            default :
                return abort(404);
        }
    ...
    public function export($type)
    {
        ...
        exportExcel('Enquiry Requests '.$file_name,'enquiry',$wherein);
    }
}
