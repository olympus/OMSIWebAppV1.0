...
You have '.$left_attempt.' login attempts remaining.';
    ...
    public function forgetpwd_otp_verifyOld1Jan2026(Request $request){
...
You have '.$left_attempt.' attempts remaining';
    ...
    public function forgetpwd_otp_verify(Request $request)
    {
        ...
        if ($request->type == 'account') {
            ...
            if (isset($customer->otp_code) && isset($request->mobile_otp)) {
                if ($customer->otp_code != $request->mobile_otp) {
                    return response(json_encode([
                        'status_code' => 403,
            ...
            if (isset($customer->otp_code) && isset($request->email_otp)) {
                if ($customer->otp_code != $request->email_otp) {
                    return response(json_encode([
                        'status_code' => 403,
            ...
            /* 🔴 ORIGINAL OTP CHECK (UNCHANGED) */
            if ($customer->otp_code != $request->otp_code) {
                return response(json_encode([
                    'status_code' => 403,
                    'message' => 'The OTP entered did not match. Please try again.'
                ]), 200)->header('Content-Type', 'text/plain');
            }
            ...
            if (strtotime($customer->valid_upto) < strtotime(now())) {
                ...
                $customer->otp_code = mt_rand(100000, 999999);
                $customer->valid_upto = now()->addMinutes(10);
                $customer->save();
        ...
        if (strtotime($customer->valid_upto) < strtotime(now())) {
            ...
            Mail::to($customer->email)->send(new SendForgotOtp($customer));
            send_sms('request_created', $customer, "", "");
        ...
        /* 🔴 SUCCESS */
        UserLoginAttemptCount::where('user_id', $customer->id)
            ->update(['forget_pwd_otp_attempts' => 0]);
